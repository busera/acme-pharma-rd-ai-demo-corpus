...
IIA_Type: CTX
IIA_Type_Rational: This document provides an overview of the cloud architecture for
  the AI sandbox, offering context about the environment, controls, and open questions
  relevant to the audit objective.
Quality_Rating: M
Relevance_Score: H
Fact: The sandbox uses a segregated cloud account with object storage, a managed vector
  index, an application layer, and an approved model endpoint. Access is restricted
  to pilot users, the R&D AI product owner, and named support engineers. Control assumptions
  include separate environments, identity federation with role-based access, encryption
  at rest and in transit, central logging, no direct public access, network controls,
  and automated data deletion. The architecture does not yet specify how prompt history,
  retrieved passages, model outputs, embeddings, or error traces are stored and classified.
  It is unclear whether vendor support personnel can access logs or payloads. Audit-relevant
  questions include whether embeddings are treated as derived confidential data, whether
  prompts and outputs are logged for traceability, and whether sandbox-to-production
  promotion controls are defined.
Summary: The document outlines the cloud architecture for ACME Pharma's R&D AI sandbox.
  The environment uses a segregated cloud account with object storage for approved
  pilot documents, a managed vector index, an application layer for retrieval and
  summarization, and an approved model endpoint. Access is limited to pilot users,
  the product owner, and named support engineers. Control assumptions include separate
  development, test, and pilot environments, identity federation with role-based access,
  encryption at rest and in transit, central logging, no direct public access, network
  egress controls, and automated data deletion after the retention period. The architecture
  does not yet address how prompt history, retrieved passages, model outputs, embeddings,
  or error traces are stored and classified, nor whether vendor support personnel
  can access logs or payloads. The document emphasizes the need for a cloud expert
  to assess the technical design and for audit planning to verify that specialist
  review occurred, high-risk assumptions are documented, and architecture evidence
  supports claimed data restrictions. It lists architecture assumptions such as a
  segregated project workspace, approved identity federation, restricted network paths,
  encrypted object storage, a retrieval index, model endpoint access, application
  and security logs, and a review queue for flagged outputs. Audit-relevant questions
  raised include whether embeddings are treated as derived confidential data, whether
  prompts and outputs are logged for traceability without over-retaining sensitive
  payloads, whether administrators or supplier support can view restricted R&D content,
  whether model endpoint calls are restricted to approved regions and data classes,
  and whether sandbox-to-production promotion controls are defined. The architecture
  components include a private application layer, a managed retrieval store, approved
  model endpoints, and a workspace interface for researchers, with source documents
  uploaded into controlled project workspaces and text extraction and indexing occurring
  inside the approved environment.
Section_Context: Strategic/operational context
IIA_Relevance_Explanation: This document provides an overview of the cloud architecture
  for the AI sandbox, offering context about the environment, controls, and open questions
  relevant to the audit objective.
IIA_Relevance: H
IIA_Confidence: M
Notes: 'Auto-derived categorization: This document provides an overview of the cloud
  architecture for the AI sandbox, offering context about the environment, controls,
  and open questions relevan...'
---
